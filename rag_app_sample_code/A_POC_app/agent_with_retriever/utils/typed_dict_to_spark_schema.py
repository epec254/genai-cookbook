...
# MAGIC
# MAGIC `typed_dicts_to_spark_schema` converts multiple TypedDicts into a Spark schema, allowing for the combination of multiple TypedDicts into a single Spark DataFrame schema. This function enables the resulting Delta Table to reflect the schema defined in `MetadataExtractionReturnValue` and `ParserReturnValue`
# MAGIC
# MAGIC Arguments:
...
def typed_dict_to_spark_fields(typed_dict: type[TypedDict]) -> StructType:
    ...
    fields = [StructField(key, get_spark_type(value), True) for key, value in type_hints.items()]
