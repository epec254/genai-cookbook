...
# MAGIC   - extract_metadata_udf: A user-defined function that takes parsed content and extracts metadata.
# MAGIC       For example: `def extract_metadata(doc_content, modification_time, doc_bytes_length, path): return {'doc_uri': 'doc_id', 'field1': 'value'}`
...
def parse_and_extract(
  ...
  try:
    # Run the parser
    parser_output_dict = parse_file_udf(raw_doc_contents_bytes=raw_doc_contents_bytes, doc_path=doc_path)

    if parser_output_dict.get("parser_status") == 'SUCCESS':
      # Run the metadata extraction
      metadata_extraction_dict = extract_metadata_udf(doc_content=parser_output_dict['doc_content'], modification_time=modification_time, doc_bytes_length=doc_bytes_length, doc_path=doc_path)

      return {**parser_output_dict, **metadata_extraction_dict}
    else:
...
def get_json_parser_udf(
  ...
  parser_udf = func.udf(
      lambda raw_doc_contents_bytes, modification_time, doc_bytes_length, doc_path: parse_and_extract(raw_doc_contents_bytes, modification_time, doc_bytes_length, doc_path, extract_metadata_udf, parse_file_udf),
...
def load_uc_volume_to_delta_table(
  ...
  parser_udf = get_json_parser_udf(extract_metadata_udf, parse_file_udf, spark_dataframe_schema)
