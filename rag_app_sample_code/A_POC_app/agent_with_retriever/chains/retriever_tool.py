...
class Tool(ABC):
    ...
    def __call__(self, *args, **kwargs) -> Any:
        ...
        """
        raise NotImplementedError("__call__ must be overridden by the tool implementation.")

    @abstractmethod
    def get_tool_definition(self) -> Dict[str, Any]:
        """
        Returns the configuration of the Tool to be passed to the LLM.
        Must follow the OpenAI tool spec.
        """
        raise NotImplementedError("tool_definition must be overridden by the tool implementation.")
...
class VectorSearchRetriever(Tool):
    """
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        if self.config.get("description"):
              description = description + f"  This tool contains documents about {self.config.get('description')}"
    ...
    def similarity_search(
        ...
        columns = [
            self.config.vector_search_schema.get("primary_key"),
            self.config.vector_search_schema.get("chunk_text"),
            self.config.vector_search_schema.get("document_uri")
        ] + self.config.vector_search_schema.get("metadata_columns")
